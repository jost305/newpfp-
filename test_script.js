// ── Privy auth bootstrap ──────────────────────────────────────────────────
  ...
  // ── Fighter Builder (Create page) ──────────────────────────────────────
  const CP_PRESETS = {
    pepe:  { name: 'PEPE', collection: 'Pepe the Frog', emoji: '🐸', className: 'trickster', traits: 'rare, meme, legendary', hp: 110, atk: 30, def: 20, spd: 18 },
    doge:  { name: 'DOGE', collection: 'Doge NFT',      emoji: '🐕', className: 'striker',   traits: 'much wow, speed, gold',   hp: 100, atk: 35, def: 10, spd: 22 },
    bonk:  { name: 'BONK', collection: 'Bonk Army',     emoji: '🔨', className: 'tank',      traits: 'powerful, heavy, slow',   hp: 200, atk: 28, def: 30, spd: 8  },
    mog:   { name: 'MOG',  collection: 'Mog Coin',      emoji: '😺', className: 'frost',     traits: 'cool, slippery, crit',    hp: 90,  atk: 40, def: 12, spd: 20 },
    wif:   { name: 'WIF',  collection: 'dogwifhat',      emoji: '🎩', className: 'blaster',   traits: 'hat, magic, burst',       hp: 95,  atk: 38, def: 15, spd: 19 },
  };

  let cpImageDataUrl = '';

  function cpPct(v, min, max) { return Math.round(((v - min) / (max - min)) * 100); }

  function cpSync() {
    const name       = (document.getElementById('cp-name').value || 'UNNAMED FIGHTER').toUpperCase();
    const collection = document.getElementById('cp-collection').value || '—';
    const cls        = document.getElementById('cp-class').value;
    const traitsRaw  = document.getElementById('cp-traits').value;
    const traits     = traitsRaw.split(',').map(t => t.trim()).filter(Boolean);

    // heading badge
    document.getElementById('cp-class-badge').textContent = cls;
    document.getElementById('cp-preview-name-small').textContent = name;

    // preview
    document.getElementById('prev-name').textContent = name;
    document.getElementById('prev-collection').textContent = collection;
    document.getElementById('prev-class').textContent = cls.toUpperCase();
    document.getElementById('prev-traits').textContent = traits.length ? traits.join(' · ') : 'No traits yet';
  }

  function cpSyncImageUrl() {
    const url = document.getElementById('cp-image-url').value.trim();
    cpSetImage(url);
    cpSync();
  }

  function cpSetImage(src) {
    const emojiEl  = document.getElementById('cp-avatar-emoji');
    const imgEl    = document.getElementById('cp-avatar-img');
    const prevEmoji = document.getElementById('prev-emoji');
    const prevImg   = document.getElementById('prev-img');

    if (src) {
      imgEl.src = src; imgEl.style.display = 'block'; emojiEl.style.display = 'none';
      prevImg.src = src; prevImg.style.display = 'block'; prevEmoji.style.display = 'none';
    } else {
      imgEl.style.display = 'none'; emojiEl.style.display = 'block';
      prevImg.style.display = 'none'; prevEmoji.style.display = 'block';
    }
  }

  function cpSetEmoji(emoji) {
    document.getElementById('cp-avatar-emoji').textContent = emoji;
    document.getElementById('prev-emoji').textContent = emoji;
    cpSetImage('');
  }

  function cpHandleFile(event) {
    const file = event.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      cpImageDataUrl = String(reader.result);
      cpSetImage(cpImageDataUrl);
      document.getElementById('cp-image-url').value = '';
    };
    reader.readAsDataURL(file);
  }

  function cpSyncStat(key, min, max) {
    const raw = parseInt(document.getElementById(`cp-${key}`).value, 10);
    const val = Math.min(max, Math.max(min, isNaN(raw) ? min : raw));
    // Write clamped value back so deploy always reads a valid number
    document.getElementById(`cp-${key}`).value = val;
    const pct = cpPct(val, min, max) + '%';
    document.getElementById(`cp-${key}-bar`).style.width = pct;
    document.getElementById(`prev-${key}-bar`).style.width = pct;
    document.getElementById(`prev-${key}-val`).textContent = val;
  }

  // Helper: clamp + read a stat input at deploy time
  function cpReadStat(key, min, max) {
    const raw = parseInt(document.getElementById(`cp-${key}`).value, 10);
    return Math.min(max, Math.max(min, isNaN(raw) ? min : raw));
  }

  function cpLoadPreset(key) {
    const p = CP_PRESETS[key];
    if (!p) return;
    document.getElementById('cp-name').value       = p.name;
    document.getElementById('cp-collection').value = p.collection;
    document.getElementById('cp-class').value      = p.className;
    document.getElementById('cp-traits').value     = p.traits;
    document.getElementById('cp-hp').value  = p.hp;
    document.getElementById('cp-atk').value = p.atk;
    document.getElementById('cp-def').value = p.def;
  ...
  function cpDeploy() {
    ...
    if (!name) { alert('Give your fighter a name first.'); return; }
    if (!window.currentWalletAddress) {
    ...
    const payload = {
      ...
      image:      cpImageDataUrl || document.getElementById('cp-image-url').value.trim() || '',
      stats: {
