...
export const botaFighterProfileSchema = z.object({
  ...
  bantCreditsEarned: z.number().int().min(0).default(0),
  liveSpectators: z.number().int().min(0).default(0),
  liveStats: z
    .object({
      bantCreditsEarned: z.number().int().min(0).default(0),
      currentUserBantCredits: z.number().int().min(0).default(0),
      currentAgentBantCredits: z.number().int().min(0).default(0),
      simRewardBantCredits: z.number().int().min(0).default(0),
      liveSpectators: z.number().int().min(0).default(0),
